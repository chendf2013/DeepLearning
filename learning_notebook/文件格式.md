...
为了使数据集更容易与网络应用架构相匹配，将任意的数据转换为Tensor Flow所支持的格式（CSV格式和TFRecord格式）

## TFRecord格式

### 将数据转换为TFRecord格式文件

过程：先将数据填入高Example协议内存块中，再将西医内存块序列化为一个字符串，最后调用方法写入到TFRecord文件中

```
import tensorflow as tf
import numpy as np
from tensorflow.examples.tutorials.mnist import input_data
mnist = input_data.read_data_sets("/home/chendongfa/MNIST_data",

...
# cifar数据集一共60000张图片，包括50000张用于训练集（保存于data_batch_*.bin中）
# 10000张用于测试集（保存于test_batch.bin中）
# 共有十类
...
def read_cifar10(file_queue):
    ...
    # FixedLengthRecordReader类用于读取固定长度字节数信息
...
def inputs(data_dir, batch_size, distorted):
    ...
    # shuffle  用于指定是否随即打乱读文件的顺序，在实际问题中会设置为True,入队和出队的顺序会被打乱，读取速度不受影响。
    # num_epochs 限制加载初始文件列表的最大轮数。比方说num_epochs=3，代表需要将文件列表中的数据重复添加三次到文件队列中去。
    # 将队列中的文件均匀得分配给每个线程
